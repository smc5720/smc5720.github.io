...
DEV 사용자로서 이 상황을 그냥 보고만 싶지 않았습니다. dev.to의 AI 생성 기사를 줄여서 개발자들이 진짜 배움을 얻을 수 있는 공간을 만들고 싶었습니다. 그래서 Elmar와 함께 **ClassifierAI**를 만들었습니다.
...
**ClassifierAI**는 DEV의 AI 생성 콘텐츠를 즉시 감지하는 **프로토타입** 크롬 확장입니다. **TensorFlow.js**를 사용해 두 가지 기능을 수행합니다:

- **이미지 분류**: Google의 [Teachable Machine](https://teachablemachine.withgoogle.com/)으로 기사 커버 이미지가 AI 생성인지 판별합니다.
  - 학습 데이터셋: [HuggingFace AI-Generated-vs-Real-Images-Datasets](https://huggingface.co/datasets/Hemg/AI-Generated-vs-Real-Images-Datasets)
...
- **텍스트 분류**: 기사 자체가 AI 생성인지 판별합니다.
  - 사용 데이터셋: [wikipedia-human-ai](https://huggingface.co/datasets/gouwsxander/wikipedia-human-ai)
...
**커버 이미지**는 이미지 분류 모델이 분석해서 이미지 오른쪽 하단에 아이콘을 표시합니다.
...
**기사 자체**의 판별 결과는 팝업 메뉴에서 확인할 수 있습니다:
...
Elmar가 Teachable Machine에 쓰이던 **ml5.js** 라이브러리가 deprecated된 것을 발견했습니다. 문제는 수정했지만 `npm install` 시 취약점이 남아 있는 상태입니다. `npm audit fix --force`를 돌리면 오히려 더 많은 문제가 생겼습니다.
...
사용한 이미지 데이터셋은 HuggingFace의 [AI-Generated-vs-Real-Images-Datasets](https://huggingface.co/datasets/Hemg/AI-Generated-vs-Real-Images-Datasets)입니다. DEV에서 실제로 쓰이는 이미지와 꽤 다르기 때문에 모델이 완벽하진 않지만, 이전보다는 훨씬 다양한 데이터를 활용합니다.
...
UI도 개선해서 **chart.js**로 결과를 더 깔끔하게 보여줍니다.
...
GitHub Copilot은 주로 **마이그레이션 과정**에서 빛을 발했습니다:
...
Elmar Chavez: [https://dev.to/codingwithjiro](https://dev.to/codingwithjiro)
