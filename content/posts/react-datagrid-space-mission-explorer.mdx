...
작성자가 가장 마음에 들어한 지점은 패싯 필터, 그룹화, 피벗, 고정 행, 커스텀 렌더러, 내보내기를 한 화면에 다 넣고도 페이지가 따로 노는 컨트롤 더미로 변하지 않았다는 것이다.
...
작성자는 이 기능이야말로 실제 애플리케이션을 만들어보고 나서 이해가 된 쪽이라고 적었다. 중첩 컴포넌트를 직접 만들 수도 있었지만, Tree Data가 이런 구조화된 정보에 이미 자연스럽게 들어맞았다.
...
작성자가 높이 평가한 지점은 커스터마이징이 컴포넌트 기본 스타일과 싸우는 작업이 아니었다는 것이다. 대부분의 작업은 그리드를 우회하는 게 아니라 프로젝트의 시각 언어에 맞추는 쪽이었다.
...
프로젝트를 끝낸 뒤 작성자가 내린 결론은, 데이터 자체가 사용자 경험의 큰 축인 애플리케이션에 React DataGrid가 어울린다는 것이다. 구체적으로는 이런 경우다.
...
Space Mission Explorer가 좋은 테스트였던 이유는 이 조건들이 한꺼번에 겹쳤기 때문이다. 상호작용 분석용 1,200행 클라이언트 워킹셋과 서버사이드 무한 스크롤로 붙인 10만 건 라이브 아카이브가 동시에 굴러갔다.
