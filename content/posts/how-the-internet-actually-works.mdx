...
당신의 ISP도 그중 하나, 대학교 전산망도 하나, Cloudflare도 하나다. 각자 자기 케이블과 라우터를 소유하고, 특별히 누구에게 보고하지 않으며, 자발적으로 서로를 연결한다. 이 관점을 잡고 나면 인터넷의 이상한 점들이 하나씩 이해된다.
...
보다시피 상당 부분이 준비 동작이다. 실제 페이지의 첫 바이트가 움직이기도 전에 이름 조회 한 번, 3-way 핸드셰이크 한 번, 보통 그 위에 TLS 협상까지 끝내야 한다.
