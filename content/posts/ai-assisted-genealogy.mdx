...
그는 이 과정을 "소프트웨어 프로젝트처럼" 다뤘다. 그래서 이 글은 족보 이야기이자, AI 어시스턴트를 장기 과제에 붙여 쓴 실전 운영 기록이기도 하다.
...
**장시간 자율 작업.** 할당량이 리셋되기를 기다리는 데 지친 그는 Claude Max를 결제했다. 토큰이 넉넉해지자 곧 밤사이나 근무 중에 세션을 돌리기 시작했다. 이럴 때는 세션이 다 끝난 뒤에야 방향이 어긋난 걸 알아채는 일이 없도록 구조를 단단히 짜둬야 한다.
...
전문가에게는 뻔한 이야기겠지만, 처음 시작하는 사람이라면 알아둘 만한 것들이다.
