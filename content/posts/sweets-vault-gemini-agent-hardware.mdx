...
그래서 만든 게 **Sweets Vault**다. [Google Agent Development Kit(ADK)](https://github.com/google-gemini/adk)와 [Gemini API](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/start) 기반의 인터랙티브 에이전트로, 아이와 대화하고 업로드된 노트 이미지를 직접 들여다보며 독해력을 테스트한다. 모든 과제를 완료하면 간식이 담긴 서랍의 잠금장치가 실제로 열린다.
...
전체 코드는 [GitHub 저장소](https://github.com/rsamborski/sweets-vault)에서 볼 수 있다.
...
핵심 코드는 [`agent/app/agent.py`](https://github.com/rsamborski/sweets-vault/blob/main/agent/app/agent.py)에 있다. 환경 설정과 [Gemini Enterprise Agent Platform](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models)(구 Vertex AI) 초기화부터 시작한다.
...
대화형 AI에서 흔한 실패 패턴은 컨텍스트 손실이나 과제 완료 여부 환각이다. 이를 막기 위해 `ToolContext`를 사용한 명시적 상태 관리를 구현했다. 모델의 메모리에 의존하는 대신, 에이전트가 [세션 state](https://adk.dev/sessions/state/)에 완료 플래그를 직접 읽고 쓴다.
