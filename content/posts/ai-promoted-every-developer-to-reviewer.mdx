...
그리고 그 코드베이스는 애플리케이션 코드였다면 절대 통과시키지 않았을 기준으로 관리되고 있다. 테스트 커버리지 없음(11%), 리뷰어에 대한 리뷰 없음, 초록불이 기본 상태, 침묵이 성공으로 기록됨.
...
리뷰어를 테스트하라고 말하는 사람이 하룻저녁에 두 번이나 자기 리뷰어를 테스트하지 못했다. 그는 이걸 아이러니가 아니라 기본 발생률이라고 부른다. 그리고 그래서 규율보다 관례가 이긴다고 말한다.
...
정당한 반론도 있다. 네거티브 컨트롤 자체도 썩을 수 있는 테스트다. 맞는 말이다. 다만 썩은 컨트롤은 가드가 바뀌는 순간 시끄럽게 실패한다. 그 비대칭성이 컨트롤을 쓸 가치가 있게 만든다.
