...
자율 작업 하나가 공개 저장소에서 데이터를 모으고, 각 실행을 exit code로 판정했다. 어떤 실행은 멀쩡한 레코드 일곱 개를 쓴 뒤 치명적이지 않은 경고를 만나 0이 아닌 코드로 종료했다. 기계는 자기가 끝낸 작업을 "실패, 나중에 재시도"로 기록했다. 성공 아니면 실패, 둘뿐이었다. "부분 결과를 안고 중단됨"이라는 상태는 애초에 표현할 방법이 없었다. 발견은 우연이었다. 결과 파일이 그 존재를 부정하는 exit code 바로 옆 디스크에 놓여 있었다.
...
AI는 그의 업무를 옮겨놓았다. 예전에는 하루의 대부분을 산출물 만드는 데 쓰고 검증에 조금 썼다. 지금은 에이전트가 산출물 대부분을 만들고, 그의 일은 검증 그 자체다. 그렇다면 그의 판단이 실제로 새어 나가는 통로, 진짜 코드베이스는 그 가드 204개인 셈이다.
...
지금까지의 이야기는 한 문장으로 줄어든다. Neb의 팀은 이 문장을 기계적으로 적용한다.
