...
Solid의 라우터는 렌더 트리에서 아무것도 소유하지 않는다. `<Link>`도 `<A>` 컴포넌트도 없다. 대신 JSX 컴파일러가 생성하는 모든 `a[href]`와 `form[action]` 요소에 클레임(claim)을 등록한다. 서버에서 스트리밍된 콘텐츠에도 런타임이 렌더하는 서브트리 전체에 같은 클레임이 훑고 지나간다.
...
2계층을 담당하는 Solid의 프로토콜은 `"use server"` 함수 위에 서 있다. Solid는 Seroval로 경계를 넘어 직렬화하는데, 평범한 데이터뿐 아니라 Promise나 async iterator 같은 비동기 구조까지 다룬다. 서버와 클라이언트가 주고받는 것은 데이터이며, 여기엔 아직 정착하지 않은 데이터까지 포함된다.
...
이 구조를 안전하게 만드는 건 LiveView의 실패 모드가 가르쳐준 교훈이다. **서버의 라이브 그래프는 영속 상태로부터 다시 유도할 수 있는 투영이어야 한다.** 절대 진실의 출처가 아니다. 재연결이 세션이나 이벤트 리플레이처럼 프로세스와 함께 죽는 것에 의존하지 않는다.
...
위 조각들이 갖춰지면 해법 공간 전체와 그 사이의 모든 지점을 자유롭게 오갈 수 있다는 것이 이 그림의 핵심이다.
