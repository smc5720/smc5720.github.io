...
글쓴이는 벤더의 매니지드 에이전트와 자체 스캐폴드를 비교한 뒤 그 차이를 전부 모델 탓으로 돌리는 팀들을 봐왔다고 적었다. 이번 달을 지나고 나니 하네스를 통제해 바꿔보지 않은 상태에서 그런 귀속은 방어하기 어렵고, 그런 실험을 실제로 돌리는 곳은 거의 없다.

## 점수가 함께 달고 다녀야 할 것

출처가 벡터라면 벤치마크 점수에도 벡터가 필요하다. 에이전트 숫자를 인용하기 전에 최소한 이 정도는 붙어 있어야 한다는 글쓴이의 예시다. 표준이라기보다 예시에 가깝다.
