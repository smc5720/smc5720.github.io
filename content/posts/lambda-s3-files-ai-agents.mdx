...
**S3 Files**가 이 패턴을 바꾼다. S3 버킷을 로컬 파일 시스템으로 마운트하면 Lambda 코드에서 그냥 `open()`을 쓸 수 있다. 여러 Lambda 함수가 마운트된 S3 버킷을 공유 워크스페이스로 사용하는 AI 코드 리뷰 에이전트를 구축했고, 파일 접근 코드가 프로젝트에서 가장 단순한 부분이 됐다. 그게 핵심이다.
...
`s3fs`나 `smart_open` 같은 라이브러리가 이 문제를 어느 정도 완화해주지만, 이들도 내부적으로는 API 호출을 한다. 코드는 여전히 SDK를 통해 S3와 통신하는 것이지, 파일 시스템을 쓰는 게 아니다.
...
세 Lambda 함수 모두 같은 S3 버킷을 마운트한다. 오케스트레이터가 파일을 쓰고, 에이전트가 읽는다. 함수 간에 S3 키를 전달하지 않고, `/tmp`에 내려받지도 않는다. 파일 시스템이 곧 협력 레이어다.
