...
category: "news"
tags: ["programming","backend","tech","discuss"]
...
글쓴이는 디렉터리 배치가 코드 품질을 결정하기라도 하는 양 폴더 구조 논쟁에 시간을 너무 많이 쓴다고 지적한다. `utils/`를 `pkg/shared/`로 이름만 바꾼다고 뭐가 달라지지 않는다.
...
지시받는 걸 선호하는 사람에게 Go의 자유는 무섭다. 직접 고민해서 설계하고 싶은 사람에게는 말 그대로 자유고. "프레임워크가 없다"를 "생각할 필요가 없다"로 읽었다면 그때부터 함정이다.
...
클린 아키텍처의 핵심은 비즈니스 규칙을 구현 세부사항에서 떼어놓는 것이다. 파일 세 개로도 된다.
